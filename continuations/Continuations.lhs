...
> instance (Show a) => HTML (Action a) where
>   toHtml (Display h)   = h
>   toHtml (Const x)     = toHtml $ show x
>   toHtml (Form  (h,_)) = h
> 
...
>                                     (Step (Single f) cont) -> (toHtml "papp", (show (length env), cont (eval f post)):env)
